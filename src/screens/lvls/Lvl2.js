  ...
  useEffect(() => {
    const interval = setInterval(() => {
      const newWorms = Array.from({length: 5}).map(() => ({
        ...
        isBomb: Math.random() < 0.1, // 10% chance of being a bomb
      }));
      setWorms(newWorms);
    }, 1000);
  ...
  const handleCatch = (worm, idx) => {
    if (worm.isBomb) {
      ...
      setGlasses(prevGlasses => ({
        ...prevGlasses,
        [worm.color]:
          prevGlasses[worm.color] <= 3
            ? prevGlasses[worm.color] + 1
            : prevGlasses[worm.color],
      }));
    }
    if (glasses.red === 4 && glasses.pink === 4 && glasses.yellow === 4) {
      pauseTimer();
    }
  };

  const isGameWon = Object.values(glasses).every(count => count >= 4);
            ...
            <View key={color}>
              ...
              <View
                style={[
                  ...
                  color === 'pink' && glasses.pink > 3
                    ? {
                        height: 186,
                      }
                    : {
                        height: 46.5 * glasses[color],
                      },
